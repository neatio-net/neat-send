<template>
    <div>
        <van-nav-bar
                :title="$t('l.account.create.title')"
        ...
        <div>
            <van-cell-group>
                <van-cell class="walletxt" :border="false" :title="$t('l.account.create.txt')"/>
                <van-field v-model="privatekey.name"
                           :placeholder="$t('l.account.create.txt1')"/>
                <div v-if="passshow">
                    <van-cell class="walletxt" :border="false" :title="$t('l.account.create.txt2')"/>
                    <van-field v-model="privatekey.pass1" type="password"
                               :placeholder="$t('l.account.create.txt3')"/>
                    <van-cell class="walletxt" :border="false"
                              :title="$t('l.account.create.txt4')"/>
    ...
    methods: {
      ...
      async create() {
        ...
        let model = {
          ...
          encrypt: this.$g.encrypt(account.privateKey, this.privatekey.pass1),
          imgis: true,
          img: img,
          list: [
            {
